...
router.post('/', async (req: AuthRequest, res: Response) => {
  const { nome, telefone, endereco, email, observacao } = req.body;
  if (!nome) { res.status(400).json({ error: 'Nome é obrigatório' }); return; }
  const [result] = await pool.execute<any>(
    'INSERT INTO clientes (nome, telefone, endereco, email, observacao) VALUES (?, ?, ?, ?, ?)',
    [nome, telefone || null, endereco || null, email || null, observacao || null],
  );
...
router.put('/:id', async (req: AuthRequest, res: Response) => {
  const { nome, telefone, endereco, email, observacao } = req.body;
  await pool.execute(
    'UPDATE clientes SET nome=?, telefone=?, endereco=?, email=?, observacao=? WHERE id=?',
    [nome, telefone || null, endereco || null, email || null, observacao || null, req.params.id],
  );
  const [rows] = await pool.execute<any[]>('SELECT * FROM clientes WHERE id = ?', [req.params.id]);
  res.json(rows[0]);
});
